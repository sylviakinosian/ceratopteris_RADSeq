...
x <- as.data.frame(matrix(ncol = 26, nrow = 50))
x[,1:2] <- k2
x[,3:5] <- k3
x[,6:9] <- k4
x[,10:14] <- k5
...
plot_chains_species(kqlist = k6list, xlabel = x_labels)
...
pdf('k2-6_species.pdf', width = 10, height = 9)
plot_chains_species(kqlist = k6list, xlabel = x_labels)
dev.off()
...
cols <- c('#B862D3', '#000000', '#fff400', '#A8FFFD', '#A39D9D', '#69C261', '#69C261')
...
# plot painted tree with clade labels
plot(painted, ftype = "off", lwd = 2)
alt_t <- force.ultrametric(tree = t)
...
left[sapply(nodes, function(x, y) which(y==x), y = n$trees[[1]]$edge[,2])] <- '#fff400'

# gaudichaudii
nodes <- getDescendants(n$trees[[1]], 86)
left[sapply(nodes, function(x, y) which(y==x), y = n$trees[[1]]$edge[,2])] <- '#A8FFFD'

## right side, tetrad tree
right <- rep("black", nrow(n$trees[[2]]$edge))

# pteridoides 
nodes <- getDescendants(n$trees[[2]], 97)
right[sapply(nodes, function(x, y) which(y==x), y = n$trees[[2]]$edge[,2])] <- '#B862D3'

# cornuta
nodes <- getDescendants(n$trees[[2]], 96)
right[sapply(nodes, function(x, y) which(y==x), y = n$trees[[2]]$edge[,2])] <- '#69C261'

nodes <- getDescendants(n$trees[[2]], 81)
right[sapply(nodes, function(x, y) which(y==x), y = n$trees[[2]]$edge[,2])] <- '#69C261'

# thalictroides 1
nodes <- getDescendants(n$trees[[2]], 57)
right[sapply(nodes, function(x, y) which(y==x), y = n$trees[[2]]$edge[,2])] <- '#A39D9D'

# thalictroides 2
nodes <- getDescendants(n$trees[[2]], 90)
right[sapply(nodes, function(x, y) which(y==x), y = n$trees[[2]]$edge[,2])] <- '#fff400'

# gaudichaudii
nodes <- getDescendants(n$trees[[2]], 84)
right[sapply(nodes, function(x, y) which(y==x), y = n$trees[[2]]$edge[,2])] <- '#A8FFFD'
...
write.nexus(r, t, file = "2trees.nex")
my.trees <- read.nexus("2trees.nex")
rf <- multiRF(my.trees)
...
pdf("phy0.pdf", width = 15, height = 13)
plot(t, direction = "up", show.tip.label = FALSE)

# thalictroides 1
willeerd.tiplabels(tip = c(5, 27:50), pch = 21, col = "#000000", bg = "#AAFFC3", cex = 2)

# thalictroides 2
willeerd.tiplabels(tip = c(19:26), pch = 21, col = "#000000", bg = "#000075", cex = 2)

# cornuta
willeerd.tiplabels(tip = c(2:4, 6), pch = 21, col = '#000000', bg = "#F58231", cex = 2)

# pteridoides
willeerd.tiplabels(tip = c(7, 9:12), pch = 21, col = '#000000', bg = "#FFE119", cex = 2)

# gaudichaudii
willeerd.tiplabels(tip = c(1, 13:18), pch = 21, col = '#000000', bg = "#E6194B", cex = 2)

# Acrostichum
willeerd.tiplabels(tip = 8, pch = 21, col = '#000000', bg = "#800000", cex = 2)
...
c = ipa.bucky(
	name="buckytest",
	data="/media/kaiser/skinosian/paired/no-r/nr_outfiles/nr.alleles",
